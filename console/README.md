...
Port 8080 taken on the host (e.g. Zitadel)? `CONSOLE_PORT=8090 docker compose up`
— still loopback-only.
...
docker compose run --rm console cli run host:update host=srv1
docker compose run --rm console ansible-playbook playbooks/update.yml --check   # raw escape hatch
```

Target repo defaults to `../ansible-01`; override with `CONSOLE_TARGET=path docker compose up`.

## Run (bare venv fallback, Linux/macOS)

```sh
uv sync && uv run python -m console serve        # binds 127.0.0.1:8080
CONSOLE_TARGET_DIR=../ansible-01 uv run python -m console run fleet:ping
```
